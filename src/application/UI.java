...
public class UI {
	...
	private static void printCapturedPieces(List<ChessPiece> caputred) {
		List<ChessPiece> white = caputred.stream().filter(x -> x.getColor() == ChessColor.WHITE).collect(Collectors.toList());
		List<ChessPiece> black = caputred.stream().filter(x -> x.getColor() == ChessColor.BLACK).collect(Collectors.toList());
